...
header h1{font-size:1.4rem;font-weight:800;letter-spacing:-0.5px;color:#0f172a}
header p{font-size:.8rem;color:#64748b;margin-top:2px}
.card{background:#fff;border-radius:16px;padding:20px;margin-bottom:12px;box-shadow:0 1px 3px rgba(0,0,0,.06),0 1px 2px rgba(0,0,0,.04)}
.last-card{text-align:center;padding:16px 20px}
.last-label{font-size:.8rem;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px}
.last-number{font-size:3rem;font-weight:800;line-height:1;transition:color .4s}
.last-number.green{color:#10b981}
.last-number.amber{color:#f59e0b}
.last-number.red{color:#ef4444}
.last-desc{font-size:.85rem;color:#94a3b8;margin-top:2px}
.conn-banner{display:none;background:#ef4444;color:#fff;text-align:center;padding:14px 16px;border-radius:14px;margin-bottom:12px;font-size:.9rem;line-height:1.4;box-shadow:0 2px 12px rgba(239,68,68,.4)}
.conn-banner .icon{font-size:1.4rem}
.conn-banner.show{display:block;animation:alertPulse 1.5s ease-in-out 3}
@keyframes alertPulse{0%{transform:scale(1);box-shadow:0 2px 12px rgba(239,68,68,.4)}50%{transform:scale(1.02);box-shadow:0 4px 24px rgba(239,68,68,.6)}100%{transform:scale(1);box-shadow:0 2px 12px rgba(239,68,68,.4)}}
.status-card{text-align:center}
.status-indicator{display:inline-flex;align-items:center;gap:10px;background:#f8fafc;padding:10px 24px;border-radius:40px;margin-bottom:12px}
...
.distance-text{font-size:.85rem;color:#64748b;margin-top:4px}
.distance-text span{font-weight:600;color:#1e293b}
.distance-slider{padding:6px 0}
...
.chart-card h3{font-size:.85rem;color:#64748b;margin-bottom:10px;text-transform:uppercase;letter-spacing:.5px}
...
.history-count{text-align:center;font-size:.75rem;color:#94a3b8;margin-top:6px}
.stats-card{display:flex;gap:10px}
.stat{flex:1;text-align:center;padding:12px 8px;background:#f8fafc;border-radius:12px}
.stat-label{display:block;font-size:.72rem;color:#94a3b8;margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px}
.stat-value{display:block;font-size:1.2rem;font-weight:700;color:#0f172a}
.arm-btn{display:block;width:100%;padding:16px;border:none;border-radius:14px;background:#2563eb;color:#fff;font-size:1rem;font-weight:700;cursor:pointer;transition:background .2s,transform .1s;margin-bottom:12px}
...
.footer{text-align:center;padding:4px 0 20px;color:#94a3b8;font-size:.78rem;line-height:1.7}
.footer .version{font-size:.68rem;color:#cbd5e1;margin-top:2px}
.stale{opacity:.3;pointer-events:none;transition:opacity .5s}
.stale .status-dot{background:#94a3b8!important;box-shadow:none!important}
.stale .last-number{color:#94a3b8!important}
@media(max-width:400px){.card{padding:16px}.last-number{font-size:2.5rem}.status-indicator{padding:8px 18px}.stat-value{font-size:1.1rem}#chart{height:180px}}
</style>
</head>
<body>
<div class="container">
...
<div class="conn-banner" id="connBanner"><div class="icon">&#128683;</div> MOTO MUY LEJOS &mdash; La se&ntilde;al se perdi&oacute;. Verifica que est&eacute;s cerca de la moto o conectado a <strong>180gtpul</strong></div>

<div class="card last-card">
<div class="last-label">&Uacute;ltimo dato recibido</div>
<div class="last-number" id="lastNumber">--</div>
<div class="last-desc" id="lastDesc">segundos atr&aacute;s</div>
</div>
...
<div class="card">
<h3 style="font-size:.82rem;color:#64748b;margin-bottom:10px;text-transform:uppercase;letter-spacing:.5px">Proximidad</h3>
...
<span>Muy cerca</span><span>Cerca</span><span>Medio</span><span>Lejos</span><span>Sin se&ntilde;al</span>
</div>
</div>
</div>

<div class="card chart-card">
<h3>Historial de Se&ntilde;al (&uacute;ltimas 50)</h3>
...
<div class="footer">
<span>Red: <strong><span id="ssidDisplay">180gtpul</span></strong></span> &middot; <span>IP: 192.168.4.1</span> &middot; <span>Pr&oacute;x: <span id="countdown">10</span>s</span>
<p class="version">v1.2 — Pulsar IoT</p>
</div>
</div>

<script>
(function(){
var E={dot:document.getElementById('statusDot'),label:document.getElementById('distanceLabel'),fill:document.getElementById('signalFill'),rssi:document.getElementById('rssiValue'),distText:document.getElementById('distanceText'),sFill:document.getElementById('sliderFill'),sThumb:document.getElementById('sliderThumb'),clients:document.getElementById('clientCount'),uptime:document.getElementById('uptimeDisplay'),cd:document.getElementById('countdown'),btn:document.getElementById('armBtn'),histCnt:document.getElementById('historyCount'),banner:document.getElementById('connBanner'),lastNum:document.getElementById('lastNumber'),lastDesc:document.getElementById('lastDesc'),ssid:document.getElementById('ssidDisplay')};
var canvas=document.getElementById('chart'),ctx=canvas&&canvas.getContext('2d');
var countdown=0,armed=false,history=[],MAX_H=500,lastDataTime=0,STALE_T=13000,stale=false,GAP_T=30000,CHART_N=50;

function setStale(){if(stale)return;stale=true;E.banner.classList.add('show');document.querySelectorAll('.card').forEach(function(c){c.classList.add('stale')});E.btn.style.opacity='.3';E.dot.className='status-dot'}
...
function loadH(){try{var r=localStorage.getItem('pulsar_history');if(r)history=JSON.parse(r)}catch(e){}if(!Array.isArray(history))history=[];history=history.filter(function(p){return typeof p.r==='number'});for(var i=0;i<history.length;i++){if(!history[i].ts){var p=history[i];if(p.d&&p.t){history[i].ts=new Date(p.d+'T'+p.t).getTime()}else{history[i].ts=Date.now()-((history.length-i)*10000)}}}}

function addP(rssi,meters){
var ts=Date.now(),n=new Date(),h=n.getHours(),m=n.getMinutes(),s=n.getSeconds();
var t=(h<10?'0':'')+h+':'+(m<10?'0':'')+m+':'+(s<10?'0':'')+s;
var d=n.getFullYear()+'-'+((n.getMonth()+1)<10?'0':'')+(n.getMonth()+1)+'-'+(n.getDate()<10?'0':'')+n.getDate();
history.push({ts:ts,t:t,d:d,r:rssi,m:meters});saveH();
}
...
var W=canvas.width,H=canvas.height,pt=15,pb=24,pl=38,pr=38,pw=W-pl-pr,ph=H-pt-pb;
ctx.clearRect(0,0,W,H);
var pts=history.slice(-CHART_N),N=pts.length;
if(N<2){ctx.fillStyle='#94a3b8';ctx.font='14px sans-serif';ctx.textAlign='center';ctx.fillText('Esperando datos...',W/2,H/2+5);return}
var t0=pts[0].ts,t1=pts[N-1].ts,tr=t1-t0;if(tr<5000)tr=5000;
...
var dist=Math.pow(10,(grd[i]+40)/-25);
ctx.fillStyle='#94a3b8';ctx.textAlign='left';
if(dist>99){ctx.fillText(Math.round(dist)+'m',W-pr+4,y+3)}else{ctx.fillText((dist<10?dist.toFixed(1):Math.round(dist))+'m',W-pr+4,y+3)}
}
ctx.strokeStyle='#e2e8f0';ctx.beginPath();ctx.moveTo(pl,pt);ctx.lineTo(pl,pt+ph);ctx.lineTo(W-pr,pt+ph);ctx.stroke();
// RSSI line (blue) with gap detection
ctx.strokeStyle='#2563eb';ctx.lineWidth=2;ctx.lineJoin='round';
ctx.beginPath();
var started=false;
for(var i=0;i<N;i++){
var x=pl+((pts[i].ts-t0)/tr)*pw;
var r=pts[i].r;if(r<-100)r=-100;if(r>-30)r=-30;
var y=pt+((-30-r)/70)*ph;
if(i>0&&pts[i].ts-pts[i-1].ts>GAP_T){ctx.stroke();ctx.beginPath();started=false}
if(!started){ctx.moveTo(x,y);started=true}else{ctx.lineTo(x,y)}
}
ctx.stroke();
// Distance line (green) with gap detection
...
ctx.stroke();
// X time labels (2 labels)
ctx.fillStyle='#94a3b8';ctx.font='9px sans-serif';ctx.textAlign='center';
for(var i=0;i<N;i+=Math.max(1,Math.floor(N/3))){
var x=pl+((pts[i].ts-t0)/tr)*pw;
ctx.fillText(pts[i].t,x,pt+ph+16);
}
}
...
if(lastDataTime===0){E.lastNum.textContent='--';E.lastNum.className='last-number';E.lastDesc.textContent='esperando datos...';return}
var sec=Math.floor((Date.now()-lastDataTime)/1000);
E.lastNum.textContent=sec;
E.lastDesc.textContent='segundo'+(sec===1?'':'s')+' atr\u00E1s';
if(sec<13){E.lastNum.className='last-number green'
}else if(sec<20){E.lastNum.className='last-number amber'
}else{E.lastNum.className='last-number red'}
}
...
if(d.ssid)E.ssid.textContent=d.ssid;
...
setInterval(function(){updateLastSeen();if(lastDataTime!==0&&Date.now()-lastDataTime>STALE_T)setStale();if(countdown>0)countdown--;E.cd.textContent=countdown;if(countdown===0)fetchStatus()},1000);
fetchStatus();
})();
</script>
</body>
